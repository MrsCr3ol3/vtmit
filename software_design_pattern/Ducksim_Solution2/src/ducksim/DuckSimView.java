...
public class DuckSimView extends JPanel {
	...
	public void paintQuack(Graphics g) {
		...
		String quack = model.getDuck(currentDuck).performQuack();
	...
	public void paintFlyingDuck(Graphics g) {
		...
		if (animationX == 800) {
			flyTimer.stop();
			model.getDuck(currentDuck).swim();
			while (currentDuck - 1 >= 0) {
			    model.getDuck(currentDuck -1 ).swim();
			    currentDuck--;
			}
	...
	public void paintDuck(Graphics g, int pos) {
		...
		if (model.getDuck(pos).isOnDSWC()) {
			...
			else if (model.getDuck(pos).getState() == State.SWIMMING && !model.getDuck(pos).isFree()) {
